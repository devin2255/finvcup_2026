...
PRECOMPUTE_ARGS=(
  ...
  --local_model "${PROJECT_ROOT}/models/vap_mc_state_dict_ch_kyoto_10hz_20000msec.pt"
  --out_dir "${CACHE_DIR}"
  --bc_enabled
  --bc_lang ch
  --bc_mode bc
...
if [[ "${VAP_OVERWRITE}" != "1" ]] && cache_ok; then
  echo "[dualvapbc-gated] reuse existing VAP+BC cache"
else
  echo "[dualvapbc-gated] cache missing/bad or overwrite requested; precompute VAP+BC"
  if [[ -z "${MAAI_PYTHON}" ]]; then
    if [[ -x /opt/maai-env/bin/python ]]; then
      MAAI_PYTHON=/opt/maai-env/bin/python
    else
      MAAI_PYTHON="${TRAIN_PYTHON}"
    fi
  fi
  echo "[dualvapbc-gated] precompute python=${MAAI_PYTHON}"
  "${MAAI_PYTHON}" "${PRECOMPUTE_ARGS[@]}"
  cache_ok
fi

echo "[dualvapbc-gated] train"
"${TRAIN_PYTHON}" -m src.train --config "${CONFIG}"
