...
私域测试集（上下文 (0,30]\,s 动态变长）上取得 Macro-F1 \todo{填分数}。
\end{abstract}
...
\section{实验结果}
主指标为五类事件的 \textbf{Macro-F1}。% 请填入你的实际结果
\begin{table}[h]
\centering\footnotesize
\caption{私域测试集结果（占位，待填）}
\begin{tabular}{lccccc c}
\toprule
方法 & C & NA & I & BC & T & Macro-F1 \\
\midrule
单模型(best) & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
\textbf{Top-5 集成} & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} & \textbf{\todo{}} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{消融（占位，待填）}：
\begin{table}[h]
\centering\footnotesize
\begin{tabular}{l c}
\toprule
