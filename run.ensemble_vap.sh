...
BC_LOCAL=/app/models/vap-bc_state_dict_ch_10hz_20000msec.pt
MAAI_DIR=/app/MaAI

t0=$(date +%s)

# -------- 阶段 A：VAP 预计算（maai-env，多进程并行，单帧版） --------
...
# 总预算 60min 大头在这：每模型 ~8-12min，5 个串行 ~40-60min。
# ENSEMBLE_TOPK 设非空只用前 K 个最强成员（manifest 已按指标降序）；时间紧设 3（~30min）。
