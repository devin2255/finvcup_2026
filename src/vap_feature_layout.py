"""Shared helpers for VAP and BC feature cache layouts."""
...
def flat_vap_result(result: dict) -> List[float]:
    """Flatten one MaAI VAP result into the stable 18-dim layout."""
...
def flat_bc_result(result: dict) -> float:
    """Extract scalar BC probability from one MaAI BC result."""
...
def bc_tail_summary(values: Iterable[float], frame_rate: float, tail_sec: float) -> np.ndarray:
...
def append_bc_tail_features(
    ...
    """Append causal [last, max_tail, mean_tail] BC features to each VAP frame."""
...
def append_bc_tail_to_last_feature(
    ...
    return np.concatenate([vap_arr, bc_tail_summary(bc_values, frame_rate, tail_sec)]).astype(
        np.float32,
        copy=False,
    )
